  ...
  <article class="border">
    <button v-on:click="toSinglePostView(this.post.id, this.$router)">
      ...
      <audio v-if="post.recordingcontent != null" controls>
        <source
          v-if="post.recordingcontent != null"
          :src="post.recordingcontent"
          type="audio/mp3"
        />
      </audio>
      ...
      <div class="postHeader">
        <button v-on:click="updatePost()">
          <img
            src="https://static.wikia.nocookie.net/spongebob/images/8/84/Krabby_Patty_icon.png"
            width="30"
            alt="like button"
          />
        </button>
...
export default {
  ...
  props: ["post"],
  methods: {
    toSinglePostView(postId, router) {
      ...
      fetch(`http://localhost:3000/api/posts/${postId}`, {method: "GET"})
        .then(function (response) {
          return response.json();
        })
        .then(function (data) {
          console.log(data)
          router.push({name: "PostPage", state: {data: data} })
        })
